#!/usr/bin/perl -w
package parse_hmmsearch_results
use strict;
use Getopt::Long;
use MRC;
use MRC::Run;
...
# my $resultfile = "/bueno_not_backed_up/sharpton/hmmsearch/family_batch_ALL-seed_seqs_ALL.domtblout.hse.gz";
...
# my $evalcut  = 0.00001;
# my $coverage = 0.8;
# my $fci_check = 1; #do a lighthouse db lookup to ensure that sequences are from proper fci families
# my @fcis      = ( 4, 6 );
...
sub main {
	...
	my $batchfile   	= $args{batchfile};
    my $output			= $args{output};
    my $evalcut			= $args{evalcut};
    my $coverage 		= $args{coverage};
    my $good_fci_seqs 	= $args{good_fci_seqs};
    my $tophit 			= $args{tophit};
    my $screen_prom_seqs= $args{screen_prom_seqs};
    my $prom_seq_val	= $args{prom_seq_val};
...
while(<IN>){
	...
	if( $query ne $current_q ){
	    $has_hits{$current_q}++;
	    process_query( $current_q, \%results, $out );
	    %results = ();
	    $current_q = $query;
	}
    }
...
sub process_query{
    ...
    my $members_rs = $project->MRC::DB::get_family_members_by_famid( $hmm );
    #convert to a hash - will be faster because of the way we use indexing
    my %members = ();
    while( my $member = $members_rs->next() ){
	if( defined( $member->gene_oid ) ){
	    my $member_id = $member->gene_oid->gene_oid;
	    $members{$member_id}++;
	}
    }
    #initialize a few vars for the family
    my $n_members       = keys( %members );
    my $member_hits     = 0;
	...
	foreach my $hit( keys( %{ $results->{$hmm} } ) ){
	    print "\tFound hit: $hit\n";
		...
		my $is_member = check_if_member( \%members, $hit );
		if( $is_member ){
		    print "\t\t...is a member\n";
		    $member_hits++;
...
sub check_if_member{
    my $members   = shift; #ref hash of member ids
    my $id        = shift;
    my $is_member = 0;
