...
sub gather_CDS {
	...
	my $limit		= $args{limit};			#limit on db select for development
	...
	if ($old){
		$prepare_statement = "SELECT genes.gene_oid, genes.protein, familymembers.famid FROM genes natural join familymembers WHERE genes.type=\'CDS\' ";

	}
	else {
		$prepare_statement = "SELECT g.gene_oid, g.protein FROM genes g WHERE g.type=\'CDS\' AND g.gene_oid ";
		
		# $prepare_statement .= "NOT " unless $old;
		$prepare_statement .= "IN (SELECT f.gene_oid FROM familymembers f)" unless $old;
	} 


	#limit the number of records
	$prepare_statement .= " LIMIT ${limit}"  if $limit;
